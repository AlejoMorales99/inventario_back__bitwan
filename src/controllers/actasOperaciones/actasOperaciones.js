...
async function postCrearActaOperaciones(req, res)  {
    try {
      ...
      if (data.code == 200) {
        ...
        const ncajanap = req.body.ncajanap;
      ...
      if (req.file) {
        ...
        try {
          ...
          const condicion = await actasOperacionesModal.postCrearActaOperaciones(RazonMovimiento,numServicioActa,usuarioCreoActa,ImgGuia,descripcion,fechaFormateada,idAgenda,ncajanap);
...
//funcion que actualiza la caja de un acta de operaciones
async function actualizarNumCaja(req, res)  {
  try {
    ...
    if (data.code == 200) {
      ...
      const idOperacion = req.body.idOperacion;
      const cambioCaja = req.body.cambioCaja;
      const tecnico_o_administractivo = req.body.tecnico_o_administractivo;
     
      try {

        const condicion = await actasOperacionesModal.actualizarNumCaja(idOperacion,cambioCaja,tecnico_o_administractivo);
